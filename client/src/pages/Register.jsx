    ...
    <div style={{ minHeight: '100vh', background: '#fafaf9', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
        <div style={{ background: '#fff', border: '1px solid #e8e7e4', borderRadius: '8px', padding: '32px', width: '100%', maxWidth: '380px' }}>
          ...
          <p style={{ fontSize: '13px', color: '#6f6e69', marginBottom: '24px' }}>Get started with ClientPortal</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {[['Full name','name','text','John Doe'],['Email','email','email','you@example.com'],['Password','password','password','••••••••']].map(([label,key,type,ph]) => (
              <div key={key}>
                <label style={{ fontSize: '12px', fontWeight: 500, color: '#6f6e69', display: 'block', marginBottom: '5px' }}>{label}</label>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 500, color: '#6f6e69', display: 'block', marginBottom: '5px' }}>Role</label>
          ...
          <p style={{ textAlign: 'center', fontSize: '13px', color: '#6f6e69', marginTop: '20px' }}>
            Already have an account? <Link to="/login" style={{ color: '#111', fontWeight: 500, textDecoration: 'underline' }}>Sign in</Link>
          </p>
        </div>
      </div>
    </div>
